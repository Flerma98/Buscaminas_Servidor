...
public class Servidor {
    ...
    private static int filas_juego = 19;
    private static int columas_juego = 19;
    private static int minas_division = 9;

    static JFrame frame = new JFrame("Servidor Buscaminas");
    static JTextField txtIP = new JTextField(35);
    static JTextArea messageArea = new JTextArea(35, 40);
        ...
        public void run() {
            try {
                ...
                while (true) {
                    ...
                    synchronized (lista_lobbys) {
                        ...
                        if (!yaExiste(nombre, indice)) {
                            ...
                            if (lista_lobbys.get(indice).getLista_usuarios().size() == 1) {
                                out.println("ADMIN");
                            }
                            ...
                            messageArea.setCaretPosition(messageArea.getText().length());
                            enviarLista(indice);
                            if (lista_lobbys.get(indice).getLista_usuarios().size() == Jugadores_Maximos) {
                                lista_lobbys.add(new Lobbys(new ArrayList<Usuarios>(), null, UUID.randomUUID().toString()));
                            }
                            break;
                        }
                ...
                while (true) {
                    ...
                    } else if (input.equals("/AGREGAR_MINA")) {
                        if (buscarUsuario(nombre, buscarLobby(lobby)) == lista_lobbys.get(buscarLobby(lobby)).getLista_usuarios().size() - 1) {
                            synchronized (lista_lobbys.get(buscarLobby(lobby)).getTablero()) {
                                agregarBandera(buscarLobby(lobby));
                            }
                        }
                    }
                }
    ...
    public static void finalizarPartida(int lobby) {
        for (Usuarios usuario_mgs : lista_lobbys.get(lobby).getLista_usuarios()) {
    ...
    public static void limpiarTablero(int lobby) {
        ...
        for (int x = 0; x < filas_juego; x++) {
            for (int y = 0; y < columas_juego; y++) {
                if (lista_lobbys.get(lobby).getTablero()[x][y].startsWith("Visible:bandera" + color)) {
                    lista_lobbys.get(lobby).getTablero()[x][y] = "Oculta:" + lista_lobbys.get(lobby).getTablero()[x][y].substring(lista_lobbys.get(lobby).getTablero()[x][y].indexOf("|") + 1);
                    int admin = 0;
    ...
    public static void agregarBandera(int lobby) {
        int libre = 0;
        for (String[] row : lista_lobbys.get(lobby).getTablero()) {
            for (String x : row) {
                if (!x.equals(bomba)) {
        ...
        if (libre > 0) {
            ...
            do {
                int xrandom = random.nextInt(columas_juego);
                int yrandom = random.nextInt(filas_juego);
                if (!lista_lobbys.get(lobby).getTablero()[xrandom][yrandom].equals(bomba)) {
                    lista_lobbys.get(lobby).getTablero()[xrandom][yrandom] = bomba;
                    completado = true;
                }
            } while (!completado);
            for (Usuarios usuarios : lista_lobbys.get(lobby).getLista_usuarios()) {
                usuarios.setBanderas(usuarios.getBanderas() + 1);
            }
            actualizarInformacion(lobby);
            imprimirtablero(lista_lobbys.get(lobby).getTablero());
        } else {
            limpiarTablero(lobby);
        }
    }
